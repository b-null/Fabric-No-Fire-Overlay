        ...
        HudLayerRegistrationCallback.EVENT.register(drawerWrapper -> {
            drawerWrapper.addLayer(IdentifiedLayer.of(OVERLAY_ID, (context, tickCounter) -> {
                ...
                if(config.mode == ModConfig.OverlayMode.HIDE && config.showCrosshairFireIcon && client.player != null && client.player.isOnFire() && !client.options.hudHidden && client.options.getPerspective().isFirstPerson()) {
                    ...
                    context.getMatrices().translate(centerX + 8 + (int)config.fireIconSize - offset,centerY - 2 - offset,0);
                    context.getMatrices().scale(config.fireIconSize, config.fireIconSize, 1);

                    int u = 0;
                    int v = 16 * counter;

                    context.drawTexture(RenderLayer::getGuiTextured, FIRE_TEXTURE, 0, 0, u, v, 4, 4, 16, 16, 16, 512);
